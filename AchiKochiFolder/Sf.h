...
#define LevCallee 0 // 0 for doing output, 1 for no output
...
public:
        ...
        /* [out] */ __RPC__out SHELLDETAILS *psd)
	{
		...
		if (pidl != NULL) {
			switch (iColumn) {
			case 0:
				return hr = GetDisplayNameOf(pidl, SHGDN_NORMAL, &psd->str);
			case 1:
				{
					CString s = MUt::GetRealPath(pidl);
					return SUt::Set(psd->str, s);
				}
			}
		}
...
public:
        ...
        /* [iid_is][out] */ __RPC__deref_out_opt void **ppv)
	{
		...
		*ppv = NULL;
		...
		if (S_OK == (hr = ResolvePIDL(pidl, psfDesk, pidltar, pbc))) {
			...
			ATLTRACE2(atlTraceCOM, LevCallee, "& Bound PIDL1: %s \n", static_cast<LPCSTR>(CT2A(DUt::ToStr(pidl))));
			ATLTRACE2(atlTraceCOM, LevCallee, "& Bound PIDL2: %s \n", static_cast<LPCSTR>(CT2A(DUt::ToStr(pidltar))));
			ILFree(pidltar);
			//ATLASSERT(!FAILED(hr));
			if (SUCCEEDED(hr)) {
			...
			if (pUnk != NULL) {
				CComQIPtr<IPersistFolder3> p3 = reinterpret_cast<IUnknown *>(pUnk);
				if (p3 != NULL) {
					CLSID clsid;
					if (S_OK == (hr = p3->GetClassID(&clsid))) {
						LPITEMIDLIST pidlfp = NULL;
						if (S_OK == (hr = p3->GetCurFolder(&pidlfp))) {
							PERSIST_FOLDER_TARGET_INFO pfti;
							if (SUCCEEDED(hr = p3->GetFolderTargetInfo(&pfti))) {
								LPITEMIDLIST pidlHere = ILCombine(pidlView, pidl);
								if (pidlHere != NULL) {
									CComPtr<IShellFolder> psffp;
									if (S_OK == (hr = SHCoCreateInstance(NULL, &clsid, NULL, IID_IShellFolder, reinterpret_cast<void **>(&psffp)))) {
										p3 = psffp;
										if (p3 != NULL) {
											pfti.pidlTargetFolder = pidlfp;
											if (S_OK == (hr = p3->InitializeEx(NULL, pidlHere, &pfti))) {
												pUnk->Release();
												*ppv = NULL;
												hr = psffp->QueryInterface(riid, ppv);
												ILFree(pidlfp);
												ILFree(pidlHere);
												return hr;
											}
        ...
        /* [out][in] */ __RPC__inout SFGAOF *rgfInOut)
	{
		...
		if (S_OK == (hr = ResolvePIDL(apidl[0], psfDesk, pidltar))) {
			...
			*rgfInOut |= SFGAO_LINK;
			...
			ATLTRACE2(atlTraceCOM, LevCallee, "& Wrong PIDL: %s \n", static_cast<LPCSTR>(CT2A(DUt::ToStr(apidl[0]))));
			ATLASSERT(false);
		}
	...
	public:
		static CString ToStr(LPCITEMIDLIST pidl) {
			...
			while (pidl != NULL) {
				int cb = pidl->mkid.cb;
				strErr.AppendFormat(_T("%04x "), cb);
				if (cb < 2)
					break;
				cb -= 2;
				for (int x=0; x<cb; x++) {
					strErr.AppendFormat(_T("%02x "), 0U +pidl->mkid.abID[x]);
				}
				pidl = ILNext(pidl);
			}
			return strErr;
	...
	HRESULT ResolvePIDL(PCUITEMID_CHILD pidl, CComPtr<IShellFolder> &psfDesk, PUITEMID_CHILD &pidltar, LPBC pbc = NULL) {
		...
		if (S_OK == (hr = SHGetDesktopFolder(&psfDesk))) {
			ULONG ch = 0;
			ULONG atts = 0;
			CString strPath = MUt::GetRealPath(pidl);
			if (false
				|| IDLUt::FromStr(strPath, pidltar) 
				|| S_OK == (hr = psfDesk->ParseDisplayName(NULL, pbc, const_cast<LPWSTR>(static_cast<LPCWSTR>(strPath)), &ch, &pidltar, &atts))
			) {
